...
// ── Punt-specific timing ──────────────────────────────────
export const PUNT_PRE_SNAP_MS = 800;
export const PUNT_SNAP_MS = 250;
export const PUNT_RESULT_MS = 600;
export const PUNT_POST_PLAY_MS = 200;
...
/** Punt flight phase fraction (when ball lands / is caught) */
export const PUNT_PHASE_END = 0.55;
...
/** Get the development phase duration for punts based on play outcome */
export function getPuntDevMs(play: { type: string; yardsGained: number; isTouchdown?: boolean; description?: string } | null): number {
  if (!play || play.type !== 'punt') return DEVELOPMENT_MS;
  const desc = (play.description || '').toLowerCase();
  if (play.yardsGained === 0) return 2000;          // touchback
  if (desc.includes('fair catch')) return 2200;       // fair catch
  if (play.isTouchdown) return 3500;                 // TD return
  if (Math.abs(play.yardsGained) >= 50) return 3000; // long punt
  return 2600;                                        // normal punt
}
