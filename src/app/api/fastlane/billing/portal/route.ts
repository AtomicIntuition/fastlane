...
import { createBillingPortalSession, createStripeCustomer } from '@/lib/fastlane/stripe';

const JSON_CONTENT_TYPE = 'application/json';
const MAX_PORTAL_REQUEST_BYTES = 4_096; // 4 KB
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentLengthHeader !== null) {
      ...
      if (contentLength > MAX_PORTAL_REQUEST_BYTES) {
        return NextResponse.json({ error: 'Payload too large' }, { status: 413 });
      }
    }

    const rawBody = await request.text();
    if (Buffer.byteLength(rawBody, 'utf8') > MAX_PORTAL_REQUEST_BYTES) {
      return NextResponse.json({ error: 'Payload too large' }, { status: 413 });
    }
    if (rawBody.length > 0) {
    ...
    try {
      let session;
      try {
        session = await createBillingPortalSession(customerId, `${appUrl}/fastlane/app`);
      } catch (error) {
        if (!isRecoverableCustomerError(error)) {
          throw error;
        }
        ...
        try {
          await upsertFastLaneSubscription({ userId, stripeCustomerId: customer.id });
        } catch {
          // Non-fatal: portal session can still be created for the recovered customer.
        }
        session = await createBillingPortalSession(customer.id, `${appUrl}/fastlane/app`);
      }
      if (!session.url) {
    ...
    return NextResponse.json({ error: 'Unable to initialize billing portal' }, { status: 500 });
  }
}
